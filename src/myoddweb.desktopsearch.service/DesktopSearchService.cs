...
namespace myoddweb.desktopsearch.service
{
  internal class DesktopSearchService : ServiceBase
  {
    ...
    /// <param name="config"></param>
    /// <returns></returns>
    private static IPersister CreatePersister(interfaces.Logging.ILogger logger, IConfig config )
    {
    ...
    private void LogStartupComplete( IConfig config )
    {
      ...
      sb.AppendLine($"  BusyEventsProcessorMs             {config.Processors.QuietEventsProcessorMs}Ms");
